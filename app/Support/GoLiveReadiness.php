...
class GoLiveReadiness
{
    public static function report(): array
    {
        $checks = collect([
            ...
            self::check('Planos comerciais configurados', Schema::hasTable('billing_plans') && BillingPlan::where('active', true)->count() >= 2, 'O produto precisa ter regras internas de plano, consumo e limites para operar como SaaS.', 'Billing', 'Revisar limites por uso, trial, upgrade/downgrade e alertas no painel.'),
            self::check('Uso e billing no painel', Schema::hasTable('billing_plans') && BillingPlan::where('active', true)->count() > 0, 'Nesta fase, a conversao deve vender valor primeiro e explicar plano/consumo dentro do workspace.', 'Billing', 'Fortalecer bloco de uso mensal, eventos restantes, retencao e upgrade no dashboard.'),
            ...
            self::check('Contato publico de suporte', Route::has('contact') && filled(config('devlog.support_email')), 'Usuarios, revisores e devs precisam de canal publico antes de criar conta.', 'Suporte', 'Trocar email placeholder pelo dominio oficial e definir rotina de resposta.'),
