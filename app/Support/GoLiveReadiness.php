...
class GoLiveReadiness
{
    public static function report(): array
    {
        $checks = collect([
            ...
            self::check('GitHub App oficial configurado', filled(config('services.github_app.app_id')) && filled(config('services.github_app.client_secret')) && filled(config('services.github_app.webhook_secret')) && filled(config('services.github_app.private_key_path')) && file_exists((string) config('services.github_app.private_key_path')) && Route::has('webhooks.github-app'), 'App oficial precisa de callback, webhook público, permissões mínimas e credenciais reais.', 'GitHub', 'Criar app definitivo, revisar escopos e apontar webhook público.', 'externo'),
            ...
            self::check('Roadmap público/admin atualizado', Schema::hasTable('roadmap_items') && RoadmapItem::count() >= RoadmapCatalog::expectedTotal() && RoadmapItem::where('status', 'done')->count() > 0, 'Ajuda a explicar maturidade do produto e acompanhar progresso de launch.', 'Gestão', 'Manter prioridades, percentuais e bloqueadores atualizados.', 'local'),
