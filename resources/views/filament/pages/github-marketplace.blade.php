...
<x-filament-panels::page>
  <style>
    ...
    .stack{display:grid;gap:12px}.block{border:1px solid var(--line);border-radius:14px;background:#0b1118;padding:14px}.block strong{display:block;margin-bottom:8px}.block p{color:var(--muted);line-height:1.65;margin:0}
    ...
    .check.done{border-color:rgba(105,227,154,.38);background:rgba(105,227,154,.07)}.badge{width:34px;height:34px;border-radius:12px;display:grid;place-items:center;border:1px solid var(--line);font-weight:950;color:var(--muted)}.done .badge{background:var(--green);border-color:var(--green);color:#071018}
    ...
    @media(max-width:1100px){.grid,.wide{grid-template-columns:1fr}}@media(max-width:720px){.check{grid-template-columns:auto 1fr}.check .pill{grid-column:1/-1}}
  </style>
  ...
  <div class="market">
    ...
    <section class="grid">
      <div class="card">
        ...
        <div class="stack">
          <div class="block"><strong>Nome</strong><p>GH DevLog ou GitHub DevLog AI.</p></div>
          <div class="block"><strong>Categoria</strong><p>Developer tools. Secundarias possiveis: Monitoring, Collaboration.</p></div>
          <div class="block"><strong>Short description</strong><pre class="copy">Private webhook inbox for GitHub Apps and repository events, with signature validation, sanitized payload history and workspace-based debugging.</pre></div>
          <div class="block"><strong>Full description</strong><pre class="copy">GitHub DevLog AI helps developers and teams debug GitHub webhooks with confidence. It receives GitHub App and repository webhook events, validates X-Hub-Signature-256, stores sanitized payloads in isolated workspaces and turns raw deliveries into a readable inbox.
...
It is built for developers creating GitHub Apps, SaaS teams integrating with GitHub, agencies demonstrating automations and engineering teams that need an audit trail for webhook deliveries.</pre></div>
        </div>
      </div>
      ...
      <div class="card">
        ...
        <div class="stack">
          @foreach ($links as $link)
            <div class="block"><strong>{{ $link['label'] }}</strong><p>{{ $link['value'] }}</p></div>
          @endforeach
        </div>
      </div>
    ...
    <section class="wide">
      <div class="card">
        ...
        <p class="lead">O produto nao usa GitHub apenas para autenticar usuarios. Ele recebe e valida entregas reais, vincula instalacoes a workspaces, armazena payloads sanitizados, mostra delivery IDs e oferece uma visao operacional da saude dos webhooks.</p>
      </div>
      <div class="card">
        <div class="kicker">Plano inicial sugerido</div>
        <div class="block"><strong>Starter / Free</strong><p>Para devs individuais testando fluxos de webhook e validando entregas de GitHub App. Limite mensal reduzido, retencao basica, um workspace, historico de assinaturas validas e suporte publico.</p></div>
      </div>
    </section>
  </div>
</x-filament-panels::page>
