@php
  ...
  $checks = collect([
    ['area' => 'Produto público', 'title' => 'Landing page de integração GitHub', 'done' => true, 'detail' => url('/github')],
    ['area' => 'Produto público', 'title' => 'Política de privacidade publicada', 'done' => true, 'detail' => url('/privacy')],
    ['area' => 'Produto público', 'title' => 'Termos de uso publicados', 'done' => true, 'detail' => url('/terms')],
    ['area' => 'Segurança', 'title' => 'Webhooks com assinatura HMAC validada', 'done' => $validEvents > 0, 'detail' => $validEvents.' evento(s) validado(s)'],
    ['area' => 'Segurança', 'title' => 'Payloads sanitizados antes do armazenamento', 'done' => true, 'detail' => 'Segredos e tokens são mascarados'],
    ...
    ['area' => 'Infra', 'title' => 'APP_URL em HTTPS para produção', 'done' => $hasHttps, 'detail' => $appUrl],
  ]);
...
<x-filament-panels::page>
  <style>
    .gh-ready{--ink:#f3f7fb;--muted:#9aa9b5;--line:#273544;--blue:#50b8ff;--green:#69e39a;--yellow:#ffd166;--red:#ff6b6b;color:var(--ink)}
  ...
  <div class="gh-ready">
    <section class="gh-hero">
      <div class="gh-card">
        ...
        <h1 class="title">Prontidão para lançamento oficial.</h1>
        <p class="lead">Checklist vivo para transformar o DevLog AI em uma integração apresentável ao ecossistema GitHub: produto público, segurança, SaaS, suporte, GitHub App e infraestrutura de produção.</p>
      </div>
    ...
    <section class="metrics">
      <div class="metric"><div class="value">{{ $roadmapPercent }}%</div><div class="label">roadmap concluído</div></div>
      ...
      <div class="metric"><div class="value">{{ $plansCount }}</div><div class="label">planos ativos</div></div>
    </section>
    ...
    <section class="grid">
      ...
      <aside class="gh-card">
        ...
        <div class="actions">
          <a class="action primary" href="{{ url('/github') }}" target="_blank">Revisar página pública GitHub</a>
          <a class="action" href="{{ url('/privacy') }}" target="_blank">Validar privacidade</a>
          <a class="action" href="{{ url('/terms') }}" target="_blank">Validar termos</a>
          <a class="action" href="{{ url('/admin/roadmap') }}">Abrir roadmap visual</a>
        </div>
        <div class="label" style="margin-top:16px;line-height:1.6">Para a submissão oficial, ainda precisamos criar o GitHub App real no GitHub, preencher as credenciais no `.env`, publicar em domínio HTTPS e executar testes ponta a ponta com instalação em conta/organização.</div>
      </aside>
    </section>
  </div>
</x-filament-panels::page>