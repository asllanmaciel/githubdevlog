...
<x-filament-panels::page>
  <style>
    ...
    .mark{width:34px;height:34px;border-radius:12px;display:grid;place-items:center;border:1px solid var(--line);font-weight:950;color:var(--yellow)}.done .mark{background:var(--green);border-color:var(--green);color:#071018}.pill{border:1px solid var(--line);border-radius:999px;padding:5px 9px;color:var(--muted);font-size:12px;white-space:nowrap}.pill.local{color:var(--green);border-color:rgba(105,227,154,.45)}.pill.externo{color:var(--yellow);border-color:rgba(255,209,102,.45)}.action{margin-top:7px;color:#b7e4ff;font-size:13px}
  ...
  <div class="golive">
    <section class="hero">
      ...
      <h1 class="title">Painel de decisão para colocar o DevLog AI no ar.</h1>
      <p class="lead">A preparação local está em {{ $report['local_percent'] }}%. O que ainda bloqueia o lançamento público é externo: domínio final, credenciais oficiais, GitHub App e operação real de produção.</p>
      <div class="bar"><div class="fill"></div></div>
    </section>

    <section class="metrics">
      <div class="metric"><div class="value">{{ $report['percent'] }}%</div><div class="label">prontidão total</div></div>
      <div class="metric"><div class="value">{{ $report['local_percent'] }}%</div><div class="label">preparação local</div></div>
      <div class="metric"><div class="value">{{ $report['external_percent'] }}%</div><div class="label">dependências externas</div></div>
      <div class="metric"><div class="value">{{ $report['summary']['blockers'] }}</div><div class="label">bloqueadores atuais</div></div>
      <div class="metric"><div class="value">{{ $report['ready'] ? 'sim' : 'não' }}</div><div class="label">liberado para lançar</div></div>
    </section>
    ...
    <section class="grid">
        ...
        @foreach ($report['checks'] as $check)
          <div class="check {{ $check['done'] ? 'done' : '' }}">
            ...
            <div>
              ...
              <div class="action">Próximo passo: {{ $check['nextAction'] }}</div>
            </div>
            <div class="d-flex gap-1 flex-wrap justify-content-end">
              <span class="pill">{{ $check['area'] }}</span>
              <span class="pill {{ $check['kind'] }}">{{ $check['kind'] }}</span>
            </div>
          </div>
        @endforeach
      </div>
      ...
      <aside class="card">
        <div class="kicker">Bloqueadores de lançamento</div>
        @forelse ($report['blockers'] as $blocker)
          <div class="blocker">
            <div class="d-flex justify-content-between gap-2 align-items-start">
              <strong>{{ $blocker['title'] }}</strong>
              <span class="pill {{ $blocker['kind'] }}">{{ $blocker['kind'] }}</span>
            </div>
        ...
        @empty
          <div class="check done">
            ...
            <div><strong>Nenhum bloqueador crítico</strong><div class="label">O produto está apto para preparar release público.</div></div>
