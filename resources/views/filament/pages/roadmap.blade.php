@php
  ...
  $alignment = \App\Support\RoadmapAlignment::report($roadmap);
  $alignmentById = $alignment['items']->keyBy(fn ($item) => $item['record']->id);
...
<x-filament-panels::page>
  <style>
    ...
    .devlog-pill.external{border-color:rgba(255,209,102,.42);color:var(--yellow)}.devlog-pill.local{border-color:rgba(80,184,255,.42);color:#b7e4ff}.devlog-evidence{border-top:1px solid rgba(39,53,68,.72);margin-top:10px;padding-top:10px;color:var(--muted);font-size:13px;line-height:1.5}.devlog-evidence strong{color:var(--ink)}.devlog-focus{display:grid;gap:10px}.devlog-focus-row,.devlog-stage{border:1px solid var(--line);border-radius:14px;background:#0b1118;padding:12px}.devlog-focus-row a{color:var(--blue);font-weight:850;text-decoration:none}.devlog-stage-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:10px}.devlog-stage strong{display:block;color:var(--ink);margin-bottom:4px}
    .devlog-metrics{grid-template-columns:repeat(6,1fr)}
    @media(max-width:1200px){.devlog-metrics{grid-template-columns:repeat(3,1fr)}}@media(max-width:760px){.devlog-metrics{grid-template-columns:repeat(2,1fr)}}
  </style>
  ...
  <div class="devlog-admin-skin">
    <section class="devlog-hero">
      <div class="devlog-card">
        ...
        <div class="devlog-actions">
          ...
          <a class="devlog-btn" href="{{ url('/admin/product-metrics') }}">Métricas</a>
    ...
    <section class="devlog-metrics">
      ...
      <div class="devlog-metric"><div class="devlog-value">{{ $alignment['local_pending']->count() }}</div><div class="devlog-label">pendências locais</div></div>
      <div class="devlog-metric"><div class="devlog-value">{{ $alignment['external_pending']->count() }}</div><div class="devlog-label">bloqueios externos</div></div>
    </section>

    <div class="devlog-layout">
      <aside class="devlog-side">
        <div class="devlog-card">
          <div class="devlog-kicker">Próximo foco</div>
          <div class="devlog-focus">
            @forelse ($alignment['next_focus'] as $focus)
              <div class="devlog-focus-row">
                <strong>{{ $focus['record']->title }}</strong>
                <div class="devlog-label">{{ $focus['next_action'] }}</div>
                @if ($focus['link'])
                  <a href="{{ url($focus['link']) }}">Abrir evidência</a>
                @endif
              </div>
            @empty
              <div class="devlog-focus-row">Roadmap interno alinhado. O foco agora é operação externa de launch.</div>
            @endforelse
          </div>
        </div>
        <div class="devlog-card">
          <div class="devlog-kicker">Estágios</div>
          <div class="devlog-stage-grid">
            @foreach ($alignment['stages'] as $stage => $stageReport)
              <div class="devlog-stage">
                <strong>{{ $stage }}</strong>
                <div class="devlog-label">{{ $stageReport['done'] }}/{{ $stageReport['total'] }} concluído(s)</div>
              </div>
            @endforeach
          </div>
        </div>
          ...
          <article class="devlog-phase">
            ...
            <div class="devlog-items">
              @foreach ($items as $item)
                @php($itemAlignment = $alignmentById->get($item->id))
                <form method="POST" action="{{ route('admin.roadmap.toggle', $item) }}" class="devlog-item {{ $item->status === 'done' ? 'done' : '' }}">
                  ...
                  <div>
                    ...
                    <div class="devlog-meta">
                      ...
                      @if($itemAlignment)
                        <span class="devlog-pill {{ $itemAlignment['kind'] === 'externo' ? 'external' : 'local' }}">{{ $itemAlignment['kind'] === 'externo' ? 'Externo' : 'Local' }}</span>
                        <span class="devlog-pill">{{ $itemAlignment['stage'] }}</span>
                      @endif
                    ...
                    @if($itemAlignment)
                      <div class="devlog-evidence">
                        <strong>Evidência:</strong> {{ $itemAlignment['evidence'] }}<br>
                        <strong>Próxima ação:</strong> {{ $itemAlignment['next_action'] }}
                        @if($itemAlignment['link'])
                          <br><a class="devlog-btn" style="margin-top:8px" href="{{ url($itemAlignment['link']) }}">Abrir</a>
                        @endif
                      </div>
                    @endif
                  </div>
